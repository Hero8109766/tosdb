...
import { TOSLanguage, TOSRegion, TOSRegionService } from "../tos-region";
...
import { TOSMonsterSkill } from "./monsterskill/tos-monster-skill.model";
...
export class TOSDomainRepository implements ITOSDomainRepository {
    ...
    public readonly config = {
        ...
        'monster_skills': {
            factory: json => new TOSMonsterSkill(TOSDataSet.MONSTERSKILLS, json),
            schema: { primaryKey: '$ID', indexes: ['$ID_NAME']},
        },
        'npcs': {
    ...
    find(dataset: TOSDataSet, page: CRUDPage) {
        return this
            .postDexieMessage(WorkerDexieCommand.FIND, { dataset, region: TOSRegionService.getRegion(),language:TOSRegionService.getLanguage(), page })
    ...
    findByIndex(dataset: TOSDataSet, key: string, value: number | string, forceSingle?: boolean) {
        return this
            .postDexieMessage(WorkerDexieCommand.FIND_BY_INDEX, { dataset, region: TOSRegionService.getRegion(),language:TOSRegionService.getLanguage(), key, value })
            .pipe(
                map((value: CRUDPageResult<any>) => value.result.map(value => this.config[dataset].factory(value))),
                map((value) => forceSingle ? value[0] : value)
            );
    }
    load(dataset: TOSDataSet, region: TOSRegion,language:TOSLanguage) {
